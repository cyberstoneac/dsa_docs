...
index: "index = (n-1) & spread" {
  style.fill: "#fff9c4"
  desc: "n = table capacity"
}
bucket: "bucket[index]" {
  style.fill: "#ffe0b2"
}

key.hashcode -> spread: "32-bit int"
spread.index -> index: "mixing"
index.bucket -> bucket: "O(1) average"
```
...
bucket.a -> bucket.b: "next"
bucket.b -> bucket.c: "next"
```
...
before: "capacity 16, size 12" {
  style.fill: "#bbdefb"
}
trigger: "size > 12 → resize" {
  style.fill: "#fff9c4"
}
after: "capacity 32" {
  style.fill: "#c8e6c9"
...
before.trigger -> trigger: "put()"
trigger.after -> after: "rehash all"
```
